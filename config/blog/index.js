var approot = __dirname,
	...
	cstatic = approot + '/../../CreaTe',
	static = approot + '/../../static',
	mediaroot = approot + '/../../public/',
...
exports.index = function(req, res){
	fs.readdir(articles, function(err,files){
		if (err){
			...
			res.render('allContentList', { title: 'All Content', titles: titles, settings: settings});
		}
	});
};

exports.staticMedia = function(req, res){

	res.sendFile(path.join(mediaroot + 'm/'+ req.params.mname+ '.html'));

	// console.log(path.join(mediaroot + req.params.mname, '.html'));
...
exports.cpage = function(req, res){
	var cpage = cstatic + '/' + req.params.cpage + '.txt';
	fs.readFile(cpage, 'utf-8', function(err, data){
		if (err){
			res.render('error', { title: '404', settings:settings});
		} else {
			var doc = data.toString().split("\n\n");
